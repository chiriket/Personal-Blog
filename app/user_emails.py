...
def send_subscriptions(new_subscription):
    send_email('Subscribed to Art Blog',sender=os.environ.get('MAIL_USERNAME'),recepients=[new_subscription.email_data],text_body=render_template('emails/subscribed.txt', new_subscription=new_subscription),html_body=render_template('emails/subscribed.html', new_subscription=new_subscription))
